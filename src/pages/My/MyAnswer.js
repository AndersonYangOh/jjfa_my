...
export default class MyAnswer extends React.Component {
  ...
  loadQuestions = async (page) => {
    ...
    this.lock = 1;
    try {
      const result = await Request.post(`personal/answer`, {
          ...
          <FlatList style={{ paddingLeft: 15, paddingRight: 15 }}
            ...
            renderItem={({ item }) => 
                <TouchableOpacity onPress={() => this._godetail(item)} style={{ borderRadius: px(10), paddingLeft: 15, paddingRight: 15, marginTop: px(20), backgroundColor: 'white', borderBottomWidth: px(1), paddingBottom: px(10), borderBottomColor: '#eee' }}>
                  <View style={{ borderBottomColor: '#e8e8e8', borderBottomWidth: px(1), paddingVertical: px(20) }}>
                    <Text style={{ fontSize: 15, lineHeight: px(40) }}>{item.question.title}</Text> 
                  </View>
                    <Text style={{ color: '#666666', fontSize: 13,marginTop: px(13), marginBottom: px(13) }} numberOfLines={2}>{item.answer.content}</Text>
